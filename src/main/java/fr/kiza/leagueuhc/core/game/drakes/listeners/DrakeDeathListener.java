...
public class DrakeDeathListener implements Listener {
    ...
    public void onDrakeDeath(EntityDeathEvent event) {
        ...
        if (killer != null) {
            ...
            Bukkit.broadcastMessage(drake.getColor() + "☠ " + ChatColor.WHITE + killer.getName() + 
                    ChatColor.GRAY + " a vaincu le " + drake.getDisplayName() + ChatColor.GRAY + " !");
            Bukkit.broadcastMessage("");
            ...
            Bukkit.broadcastMessage("");
            Bukkit.broadcastMessage(drake.getColor() + "☠ " + ChatColor.GRAY + "Le " + 
                    drake.getDisplayName() + ChatColor.GRAY + " a été vaincu !");
            Bukkit.broadcastMessage("");
        }
    ...
    public void onDrakeDamage(EntityDamageByEntityEvent event) {
        ...
        if (event.getDamager() instanceof Player) {
            ...
            attacker.sendMessage(drake.getColor() + drake.getName() + ChatColor.GRAY + 
                    " : " + ChatColor.RED + "❤ " + heartsRemaining + " cœurs");
        }
    }
}
